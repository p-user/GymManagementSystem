...
namespace WorkoutCatalog.Tests.Workouts.Features.Exercise.CreateExercise
{
    public class CreateExerciseHandlerTest : IClassFixture<ExerciseFixture>
    {
        ...
        public CreateExerciseHandlerTest(ExerciseFixture fixture)
        {
            ...
            var mockDb = _fixture.Exercises.CreateDbSetMock<Models.Exercise, Guid>().Object;
            _dbContextMock.Setup(x => x.Exercises).Returns(mockDb);
            var  exerciseCategoryFixture = new ExerciseCategoryFixture();

            var mockDbCategories = exerciseCategoryFixture.ExerciseCategories.CreateDbSetMock<Models.ExerciseCategory, Guid>().Object;
            _dbContextMock.Setup(x => x.ExerciseCategories).Returns(mockDbCategories);
          var muscleGroupFixture = new MuscleGroupFixture();

            var mockDbMuscles = muscleGroupFixture.MuscleGroups.CreateDbSetMock<Models.MuscleGroup, Guid>().Object;
